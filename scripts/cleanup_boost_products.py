...
def main():
    ...
    print("=" * 70)
    print("Boost Product Quality Control - Datasheet Verification")
    print("=" * 70)
    print()
    ...
    # Add datasheet status to all parts
    for part in filtered_parts:
        if part.get('part_number') in VERIFIED_BOOST_PRODUCTS:
    ...
    print(f"Datasheet coverage: {len([p for p in filtered_parts if p.get('has_datasheet')])} / {len(filtered_parts)}")
    print()
    print("Cleaned data saved to data/mock_parts.json")
